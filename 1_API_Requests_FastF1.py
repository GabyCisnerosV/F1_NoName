...
r_start, r_end = 1, 35  # Range of races in each year

##############################################################################
### Data lists I want to store:
weather_data_list = []
laps_data_list = []
    ...
    for future in as_completed(future_to_session):
        ...
        if session_data is not None:
            if 'weather' in session_data:
                weather_data_list.append(session_data['weather'])
            if 'laps' in session_data:
                laps_data_list.append(session_data['laps'])

# Convert lists to DataFrames
WeatherData = pd.concat(weather_data_list, ignore_index=True)
LapsData = pd.concat(laps_data_list, ignore_index=True)
